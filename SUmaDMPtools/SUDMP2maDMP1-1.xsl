...
<!--
    2025-05-28 *Current version 0.994: Changed contact and contact_id  again back to modified v0.97,
    but without ref. to contributor, which was also seprately changed to comply with
    maDMP-schema-1.1.json
    2024-03-12/04-16 Version 0.993: Changed contact and contact_id  again based on new DMPexport to ascertain correct "pairing" of name, mbox, contact_id/identifier and /type.  
    2024-02-26 Version 0.992: Corrected fault in FAIR evaluation sectionVIII//contactIDtypeQ2.  
    2023-12-27 Version 0.991: $contactIDstring and associates replaced by <contact> element identifiers from v0.97 as more stable, not dependent on changing output from DMP export. Made *contributor* array validate against madmp-schema-1.1 again by removing copy-namespaces attribute from <xsl:copy-of select="." copy-namespaces="no"/>.
    To-do next: Consolidate, remove unused variables/elements, e.g. $contactIDstring, $filext2mimetypeMap ...; convert from Fundref to RORid. 
    2023-04-17/2023-06-09 Keeping version 0.99, but updated fundref-map with entries for Riksantikvarieämbetet / Swedish National Heritage Board. 2023-06-09: Added *contributor* from $DMPexport by means of xsl:copy-of, checked validation against madmp-schema-1.1.  
    2023-01-18/2023-03-15 Version 0.99: Necessary change of variable "contactIDstring" to align with contact name in param $DMPexport; retroactively also for archived documents, but only now discovered through DMP113582.
               Changed <language> default when unanswered from 'und' to <xsl:value-of select="if (//sections[number/text()=9]/questions[number/text()=4]/answered = 'false') then 'eng' else ..."/>. Unanswered mandatory questions to be detected by Schematron schema SUDMP-FAIReval.sch instead.  
    2022-11-14 Version 0.98: Added <transform_dateTime><xsl:value-of select="current-dateTime()"/></transform_dateTime> to get a time stamp of last transformation performed.
               "Cleaned up" text in Section I: ReusedDatasetURL-licenseQ9/Text with local:removeHtml-function. Added FAIR Enabling Resources - FERs nanopublications identifiers to FAIR-score for doi, handle, and orcid.  
    2022-05-08 Version 0.97: necessary to change from "relative paths" in JSON output to absolute paths, since DMP Online changed document output order to arbitrary(?). Replaced, thus, references in path regexes  "sections\[\d\]" and "questions\[\d\]" with e.g. sections[number/text()=9]/questions[number/text()=3].
    2022-02-22 Version 0.96: Change from $datasetIDstring = //sections[2]/questions[2]/answer/text to $datasetIDstring = replace(//sections[2]/questions[2]/answer/text,' ','') to avoid whitespace in identifiers, and $dataset_idNum2string made conditional on matches($datasetIDstring,'^\d+$') or not, that is whether it is an integer or a string literal.
            ...
            Changed <grant_id> as first choice from  contac//funding[1]/array/grant_id/identifier with type 'other'.
...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:json="json" xmlns:html="http://www.w3.org/1999/xhtml"
    xmlns:xquery="http://www.w3.org/TR/xquery-3/" exclude-result-prefixes="xsl xs">
    ...
    <xsl:variable name="filext2mimetypeMap">
        <map>
            ...
            <entry filext="alnfaa" mimetype="text/plain" source="personalCommunication"
                date="20190701"/>
            <entry filext="ams" mimetype="application/octet-stream"
                openwith="https://www.agico.com/text/software/anisoft/anisoft.php"
                source="SND2021-44" date="20210218"/>
            ...
            <entry filext="cdf" mimetype="application/x-netcdf" source="datatypes.net"
                PUID="fmt/282" date="20210426"/>
            ...
            <entry filext="dll" mimetype="application/x-msdownload"
                mimetype2="application/vnd.microsoft.portable-executable"
                source="https://datatypes.net/open-dll-files" source2="PRONOM" date="20201027"
                PUID="fmt/899, fmt/900"/>
            ...
            <entry filext="dmp" mimetype="application/vnd.tcpdump.pcap" source="PRONOM"
                date="20200326"/>
            <entry filext="dng" mimetype="image/dng" source="https://datatypes.net/open-dng-files"
                date="20210303"/>
            ...
            <entry filext="jpg" mimetype="image/jpeg" source="https://datatypes.net/"
                date="20180712"/>
            <entry filext="jr6" mimetype="application/octet-stream"
                openwith="https://www.agico.com/text/software/remasoft/remasoft.php"
                source="   https://datatypes.net/open-jr6-files" date="20210218"/>
            <entry filext="jrn" mimetype="application/octet-stream"
                openwith="https://datatypes.net/sap-pos-file-types"
                source="https://datatypes.net/open-jrn-files" date="20210303"/>
            ...
            <entry filext="kml" applicationType="https://fileinfo.com/extension/kml"
                ...
                source="https://datatypes.net/open-kml-files" date="20221115"/>
            <entry filext="kmz" applicationType="https://fileinfo.com/extension/kmz"
                mimetype="application/vnd.google-earth.kmz"
                source="https://datatypes.net/open-kmz-files" date="20221115"/>
            ...
            <entry filext="netCDF" mimetype="application/x-netcdf" source="PRONOM" PUID="fmt/282"
                date="20210426"/>
            ...
            <entry filext="qpj" mimetype="application/octet-stream" source="https://datatypes.net/"
                date="20200331"/>
            <entry filext="rar" mimetype="application/vnd.rar" source="PRONOM" date="20201027"
                PUID="x-fmt/264(?)"/>
            ...
            <entry filext="Rmd" mimetype="text/x-gfm"
                ...
                date="20190319"/>
            <entry filext="rst" applicationType="restructuredText" mimetype="text/plain"
                source="JHOVE 1.18.1" date="20200723"/>
            ...
            <entry filext="seaview" applicationType="seaview.exe"
                url="http://doua.prabi.fr/software/seaview" mimetype="text/plain"
                source="JHOVE 1.18.1" date="20200729"/>
            ...
            <entry filext="vcf" mimetype="application/octet-stream" source="https://datatypes.net/"
                date="20200331"/>
            <entry filext="vhdr" mimetype="application/octet-stream"
                source="https://datatypes.net/open-vhdr-files" date="20210701"
                openwith="https://bids.neuroimaging.io/"/>
            <entry filext="vmrk" mimetype="application/octet-stream"
                source="https://datatypes.net/open-vmrk-files" date="20210701"
                openwith="https://bids.neuroimaging.io/"/>
    ...
    <xsl:variable name="funderName2fundRefIDmap">
        ...
        <map>
            <entry funderName="Energimyndigheten" fundRefID="501100004527" source="CrossrefAPI"
                date="20211004"/>
            ...
            <entry funderName="Hjärt-Lungfonden" fundRefID="501100003793" source="CrossrefAPI"
                date="20211004"/>
            <entry funderName="IFAU" fundRefID="501100012549" source="CrossrefAPI" date="20211004"/>
            <entry funderName="Ragnar Söderbergs stiftelse" fundRefID="100007459"
                source="CrossrefAPI" date="20211004"/>
            <entry funderName="Riksantikvarieämbetet" fundRefID="501100019975" source="CrossrefAPI"
                date="20230228"/>
            <entry funderName="Riksbankens jubileumsfond" fundRefID="501100004472"
                source="CrossrefAPI" date="20211004"/>
            <entry funderName="Rymdstyrelsen" fundRefID="501100001859" source="CrossrefAPI"
                date="20211004"/>
            <entry funderName="Statens Geotekniska institut" fundRefID="501100009244"
                source="CrossrefAPI" date="20211005"/>
            <entry funderName="Stockholm University" fundRefID="501100009244" source="CrossrefAPI"
                date="20211005"/>
            <entry funderName="Stiftelsen Olle Engkvist Byggmästare" fundRefID="501100004200"
                source="CrossrefAPI" date="20211222"/>
            <entry funderName="Swedish National Heritage Board" fundRefID="501100019975"
                source="CrossrefAPI" date="20230228"/>
            <entry funderName="VR - Swedish Research Council" fundRefID="501100004359"
                source="CrossrefAPI" date="20210914"/>
            <entry funderName="Vinnova" fundRefID="501100001858" source="CrossrefAPI"
                date="20211004"/>
            <entry funderName="Östersjöstiftelsen" fundRefID="100009050" source="CrossrefAPI"
                date="20211004"/>
        </map>
    </xsl:variable>

    <xsl:variable name="institutionList"
        >https://www.su.se/medarbetare/ekonomi/institutionslista-1.4400</xsl:variable>

    <xsl:variable name="affiliateDept2institutionNrMap">
        <map>
            <entry deptName="Archaeology and Classical Studies"
                instNrNamn="108 Institutionen för arkeologi och antikens kultur"
                source="$institutionList" date="2022-02-21"/>
            <entry deptName="Asian and Middle Eastern Studies"
                instNrNamn="172 Institutionen för Asien-, Mellanöstern- och Turkietstudier"
                source="$institutionList" date="2022-02-21"/>
            <entry deptName="Astronomy" instNrNamn="401 Institutionen för astronomi"
                source="$institutionList" date="2022-02-21"/>
            <entry deptName="Biochemistry and Biophysics (DBB)"
                instNrNamn="431 Institutionen för biokemi och biofysik (DBB)"
                source="$institutionList" date="2021-12-20"/>
            <entry deptName="Computer and Systems Sciences"
                instNrNamn="323 Institutionen för data- och systemvetenskap (DSV)"
                source="$institutionList" date="2022-02-21"/>
            <entry deptName="Child and Youth Studies"
                instNrNamn="318 Barn- och ungdomsvetenskapliga institutionen"
                source="$institutionList" date="2022-02-21"/>
            <entry deptName="Criminology" instNrNamn="312 Kriminologiska institutionen"
                source="$institutionList" date="2022-02-21"/>
            <entry deptName="Culture and Aesthetics"
                instNrNamn="106 Institutionen för kultur och estetik" source="$institutionList"
                date="2022-02-21"/>
            <entry deptName="Economics" instNrNamn="305 Nationalekonomiska institutionen"
                source="$institutionList" date="2022-02-07"/>
            <entry deptName="Economic History and International Relations"
                instNrNamn="301 Institutionen för ekonomisk historia och internationella relationer"
                source="$institutionList" date="2022-02-21"/>
            <entry deptName="Education" instNrNamn="306 Institutionen för pedagogik och didaktik"
                source="$institutionList" date="2022-02-07"/>
            <entry deptName="English" instNrNamn="150 Engelska institutionen"
                source="$institutionList" date="2021-10-26"/>
            <entry deptName="Environmental Science"
                instNrNamn="485 Institutionen för miljövetenskap (ACES)" source="$institutionList"
                date="2021-12-15"/>
            <entry deptName="Ethnology, History of Religions and Gender Studies"
                instNrNamn="103 Institutionen för etnologi, religionshistoria och genusvetenskap"
                source="$institutionList" date="2021-10-26"/>
            <entry deptName="Humanities and Social Sciences Education"
                instNrNamn="130 Institutionen för ämnesdidaktik" source="$institutionList"
                date="2022-02-07"/>
            <entry deptName="Institute for International Economic Studies (IIES)"
                instNrNamn="303 Institutet för internationell ekonomi (IIES)"
                source="$institutionList" date="2021-10-26"/>
            <entry deptName="Materials and Environmental Chemistry (MMK)"
                instNrNamn="432 Institutionen för material- och miljökemi (MMK)"
                source="$institutionList" date="2022-02-07"/>
            <entry deptName="Meteorology (MISU)"
                instNrNamn="404 Meteorologiska institutionen (MISU)" source="$institutionList"
                date="2022-02-07"/>
            <entry deptName="Organic Chemistry" instNrNamn="433 Institutionen för organisk kemi"
                source="$institutionList" date="2022-02-22"/>
            <entry deptName="Physical Geography" instNrNamn="463 Institutionen för naturgeografi"
                source="$institutionList" date="2022-02-22"/>
            <entry deptName="Physics" instNrNamn="402 Fysikum" source="$institutionList"
                date="2022-02-21"/>
            <entry deptName="Slavic and Baltic Studies, Finnish, Dutch and German"
                instNrNamn="158 Institutionen för slaviska och baltiska språk, finska, nederländska och tyska"
                source="$institutionList" date="2021-10-26"/>
            <entry deptName="Social Anthropology"
                instNrNamn="309 Socialantropologiska institutionen" source="$institutionList"
                date="2021-12-20"/>
            <entry deptName="Statistics" instNrNamn="311 Statistiska institutionen"
                source="$institutionList" date="2021-10-26"/>
            <entry deptName="Stockholm Resilience Centre"
                instNrNamn="481 Stockholms Resilienscentrum" source="$institutionList"
                date="2021-02-07"/>
            <entry deptName="Stockholm University Library"
                instNrNamn="680 Stockholms universitetsbibliotek" source="$institutionList"
                date="2021-10-26"/>
            <entry deptName="Teaching and Learning (merged from 2022-01-01)"
                instNrNamn="130 Institutionen för ämnesdidaktik" source="$institutionList"
                date="2022-02-07"/>
        </map>
    </xsl:variable>
    <xsl:variable name="datasetIDstring">
        ...
        <xsl:value-of
            select="replace(//sections[number/text() = 2]/questions[number/text() = 2]/answer/text, ' ', '')"
        />
    </xsl:variable>
    <xsl:variable name="datasetIDtype">
        <!-- <xsl:value-of select="//sections[2]/questions[3]/answer/options//text"/> -->
        <xsl:value-of
            select="//sections[number/text() = 2]/questions[number/text() = 3]/answer/options//text"
        />
    </xsl:variable>
    <xsl:variable name="contactIDtype">
        <!--   <xsl:value-of select="if (string-length(//sections[number/text()=9]/questions[number/text()=2]/answer/text) &gt; 1 and string-length(//sections[number/text()=9]/questions[number/text()=2]/answer/options//text) &gt; 1) then //sections[number/text()=9]/questions[number/text()=2]/answer/options//text else 'other'"/>
          <xsl:value-of select="if ($DMPexport//contact_id/type/text() = 'orcid') then $DMPexport//contact_id/type else //sections[number/text()=9]/questions[number/text()=2]/answer/options//text"/> 
     <xsl:value-of select="for $i in $DMPexport//contributor/name return if (//contact/name = $i and  string-length($i/following-sibling::contributor_id/type) &gt; 1)
         then $i/following-sibling::contributor_id/type else if (string-length($i/following-sibling::contributor_id/type) &lt; 1) then (//sections[number/text()=9]/questions[number/text()=2]/answer/options//text) else ()"/> -->
        <xsl:value-of
            select="//sections[number/text() = 9]/questions[number/text() = 2]/answer/options//text"
        />
    </xsl:variable>
    <xsl:variable name="contactIDstring">
        <xsl:value-of select="
                if (string-length(//sections[number/text() = 9]/questions[number/text() = 1]/answer/text) &gt; 1) then
                    (//sections[number/text() = 9]/questions[number/text() = 1]/answer/text)
                else
                    if (string-length(array/data_contact/email) &gt; 1) then
                        array/data_contact/email
                    else
                        array/principal_investigator/email"/>
        <!-- 1. <xsl:value-of select="if (contains($DMPexport//contact_id/identifier/text(),'orcid')) then substring-after($DMPexport//contact_id/identifier,'g/') else //sections[number/text()=9]/questions[number/text()=1]/answer//text"/>    
        3. <xsl:value-of select="(for $i in $DMPexport//contributor/name return if ($DMPexport//contact/name = $i and $i/following-sibling::contributor_id/type = 'orcid')  
            then substring-after($i/following-sibling::contributor_id/identifier,'org/') else if (not($DMPexport//contributor_id) or ($i/following-sibling::contributor_id/type != 'orcid')) then (//sections[number/text()=9]/questions[number/text()=1]/answer//text) else ())[1]"/>    
        2. <xsl:value-of select="for $i in $DMPexport//contributor/name return if ($DMPexport//contact/name = $i and  $i/following-sibling::contributor_id/type = 'orcid')  
            then substring-after($i/following-sibling::contributor_id/identifier,'org/')  then //sections[number/text()=9]/questions[number/text()=1]/answer//text else ()"/>   
            or <xsl:value-of select="(//sections[number/text()=9]/questions[number/text()=1]/answer//text)"/> else if ($i/following-sibling::contributor_id/type != 'orcid') else if (not($DMPexport//contributor_id)) then-->
    </xsl:variable>
    ...
    <xsl:variable name="ethicsNsensitive">
        <xsl:choose>
            ...
            <xsl:when
                test="//sections[number/text() = 5]/questions[number/text() = 2]/answer/options//text, ', but not \(a-e\)'">
                <xsl:value-of
        ...
        <dmp xmlns:xlink="http://www.w3.org/1999/xlink"
            ...
            <extension>
                ...
                <affiliationSUdepartment>
                    <xsl:variable name="affiliateText">
                        <xsl:value-of
                            select="array/plan_content/sections[number/text() = 9]/questions[number/text() = 3]/answer/text"
                        />
                    </xsl:variable>
                    <xsl:for-each select="
                            for $i in (//sections[number/text() = 9]/questions[number/text() = 3]/answer/options//text)
                            return
                            ...
                            <xsl:value-of select="
                                    ...
                                    return
                                        (if ($affiliateDept2institutionNrMap//entry[@deptName = $f]/@instNrNamn) then
                                            $affiliateDept2institutionNrMap//entry[@deptName = $f]/@instNrNamn
                                        else
                                            if ($f = 'Other') then
                                                ($affiliateText)
                                            else
                ...
                <DMPonline>
                    <template>
                        <id>
                            <xsl:value-of select="/array/template/id"/>
                        </id>
                        <title>
                            <xsl:value-of select="/array/template/title"/>
                        </title>
                    </template>
                </DMPonline>
                <Dataverse>
                    <transform_file>
                        ...
                        <file_version>0.994</file_version>
    ...
    <xsl:function name="local:removeHtmlTags" as="xs:string">
        <xsl:param name="element"/>
        <xsl:value-of
            select="normalize-space(replace($element, '&lt;[^\s][^&lt;]*>|&amp;nbsp;|&lt;p&gt;|&lt;/p&gt;|&#xA;|&#xD;',' '))"
        />
    </xsl:function>
    ...
    <xsl:template name="createRDAmaDMPhead">
        ...
        <xsl:variable name="contactName" select="$DMPexport/dmp/contact/name"/>
        <xsl:variable name="contributorName" select="$DMPexport/dmp/contributor/name"/>
        <contact>
            <name>
            ...
            <mbox>
                <xsl:value-of select="$DMPexport/dmp/contact/mbox"/>
                <!--   <xsl:value-of
                    select="
                    if (string-length(array/data_contact/email) &lt; 1) then
                    (array/principal_investigator/email)
            ...
            <contact_id>
                <identifier>
                    <xsl:choose>
                        <xsl:when
                            test="string-length(//sections[number/text() = 9]/questions[number/text() = 1]/answer/text) &gt; 1">
                            <xsl:value-of
                                select="//sections[number/text() = 9]/questions[number/text() = 1]/answer/text"
                            />
                        </xsl:when>
                        <!--  <xsl:when test="exists($contributorName)">
                        <xsl:value-of select="for $i in $contributorName return if ($i = $contactName) then $i/following-sibling::contributor_id/identifier else ()"/>
                    </xsl:when>
                    <xsl:when test="not(exists($contributorName))">
                        <xsl:value-of select="$DMPexport/dmp/contact/mbox"/>
                    </xsl:when> -->
                        <xsl:otherwise>
                            <xsl:value-of select="$DMPexport/dmp/contact/mbox"/>
                ...
                <!--    <identifier>
                    <xsl:value-of select="for $i in $contributorName return if ($i = $contactName) then $i/following-sibling::contributor_id/identifier else if (not($contributorName)) then $DMPexport/dmp/contact/mbox else ()"/>   
                ...
                <type>
                    <xsl:value-of select="
                            if (string-length(//sections[number/text() = 9]/questions[number/text() = 1]/answer/text) &gt; 1 and string-length(//sections[number/text() = 9]/questions[number/text() = 2]/answer/options//text) &gt; 1) then
                                //sections[number/text() = 9]/questions[number/text() = 2]/answer/options//text
                            else
                                'other'"/>
                </type>
                <!--  <type>
                  <xsl:choose>
                      <xsl:when test="exists($contributorName) and ($contributorName = $contactName)">
                          <xsl:value-of select="for $i in $contributorName return if ($i = $contactName) then $i/following-sibling::contributor_id/type else ()"/>
                      </xsl:when>
                      <xsl:when test="not(exists($contributorName))">
                          <xsl:value-of select="'other'"/>
                      </xsl:when>
                  </xsl:choose>  
                   <xsl:value-of select="for $i in $contributorName return if ($i = $contactName) then $i/following-sibling::contributor_id/type else if (not($contributorName)) then 'other' else ()"/>  
        ...
        <contributor>
          <array>  
            <xsl:for-each select="$DMPexport//contributor/array/name">
                <xsl:copy-of select="."/>
                <mbox>
                    <xsl:value-of select="following-sibling::mbox"/>
                </mbox>
                <contributor_id>
                    <identifier>
                        <xsl:value-of select="following-sibling::mbox"/>
                ...
                <role>
                   <array> 
                    <xsl:value-of select="following-sibling::role/array"/>
                   </array>    
                </role>
                <xsl:copy-of select="following-sibling::affiliation"/> 
                  <!--  <name><xsl:value-of select="following-sibling::afffiliation/name"/></name> 
                    <abbreviation><xsl:value-of select="following-sibling::afffiliation/abbreviation"/></abbreviation>
                    <region><xsl:value-of select="following-sibling::afffiliation/region"/></region>
                    -->
        ...
        <language>
            ...
            <xsl:value-of select="
                    if (//sections[number/text() = 9]/questions[number/text() = 4]/answered = 'false') then
                        'eng'
                    else
                        substring(//sections[number/text() = 9]/questions[number/text() = 4]/answer/options//text, 0, 4)"
            />
        </language>
        <array>
            <project>
                ...
                <start>
                    <xsl:value-of select="substring-before($DMPexport//start, 'T')"/>
                </start>
                <end>
                    <xsl:value-of select="substring-before($DMPexport//end, 'T')"/>
                </end>
                <array>
                    <funding>
                        <xsl:for-each
                            select="//sections[number/text() = 9]/questions[number/text() = 5]/answer/options//text">
                            <funder_id>
                                <identifier>
                                    <xsl:value-of select="
                                            if (contains(., 'Östersjöstift')) then
                                                '100009050'
                                            else
                                                if (contains(., 'VR')) then
                                                    '501100004359'
                                                else
                                                    if (contains(., 'Vinnova')) then
                                                        '501100001858'
                                                    else
                                                        if (contains(., 'FORTE')) then
                                                            '501100006636'
                                                        else
                                                            if (contains(., 'FORMAS')) then
                                                                '501100001862'
                                                            else
                                                                if (contains(., 'IFAU')) then
                                                                    '501100012549'
                                                                else
                                                                    if (contains(., 'Riksbank')) then
                                                                        '501100004472'
                                                                    else
                                                                        if (contains(., 'Ragnar')) then
                                                                            '100007459'
                                                                        else
                                                                            if (contains(., 'Geotekniska')) then
                                                                                '100016779'
                                                                            else
                                                                                ()"
                                    />
                        ...
                        <xsl:for-each select="
                                for $i in tokenize(//sections[number/text() = 9]/questions[number/text() = 5]/answer/text, ', ')
                                return
                                    local:removeHtmlTags($i)">
                            <funder_id>
                                <identifier>
                                    <xsl:value-of select="
                                            ...
                                            return
                                                (if ($funderName2fundRefIDmap//entry[@funderName = $f]/@fundRefID) then
                                                    $funderName2fundRefIDmap//entry[@funderName = $f]/@fundRefID
                                                else
                                                    $f)"/>
                                </identifier>
                                <type>
                        ...
                        <xsl:choose>
                            <xsl:when
                                test="string-length($DMPexport//funding/array/grant_id/identifier) &gt; 2">
                                <grant_id>
                                    <identifier>
                                        <xsl:value-of
                                            select="$DMPexport//funding/array/grant_id/identifier"/>
                                    </identifier>
                                    <type>
                                        <xsl:value-of select="'other'"/>
                                    </type>
                            ...
                            <xsl:when
                                test="string-length(//sections[number/text() = 9]/questions[number/text() = 6]/answer/text) &gt; 4">
                                <grant_id>
                                    <identifier>
                                        <xsl:value-of
                                            select="local:removeHtmlTags(//sections[number/text() = 9]/questions[number/text() = 6]/answer/text)"
                                        />
                                    </identifier>
                                    <type>
                                        <xsl:value-of select="
                                                if (contains(//sections[number/text() = 9]/questions[number/text() = 6]/answer/text, 'http')) then
                                                    'url'
                                                else
                                                    'other'"
                                        />
                        ...
                        <funding_status>
                            <xsl:value-of select="
                                    if (string-length(//sections[number/text() = 9]/questions[number/text() = 7]/answer/options//text) &gt; 2) then
                                        (//sections[number/text() = 9]/questions[number/text() = 7]/answer/options//text)
                                    else
                                        'planned'"/>
                        </funding_status>
    ...
    <xsl:template name="createRDAdatasetSec">
        ...
        <dataset>
            <array>
                <!--no. 0: head/main/start -->
                <title>
                    <xsl:value-of
                        select="local:removeHtmlTags(//sections[number/text() = 2]/questions[number/text() = 5]/answer/text)"
                    />
                </title>
                <description>
                    <xsl:value-of
                ...
                <dataset_id>
                    ...
                    <xsl:variable name="dataset_idNum2string">
                        <xsl:value-of select="
                                if (matches($datasetIDstring, '^\d+$')) then
                                    (concat('ID-', $datasetIDstring))
                                else
                                    $datasetIDstring"/>
                        <!-- <xsl:value-of
                    ...
                    <identifier>
                        <xsl:value-of select="$dataset_idNum2string"/>
                    </identifier>
                    <type>
                        <xsl:value-of select="
                                if (//sections[number/text() = 2]/questions[number/text() = 2]/answered = 'true') then
                                    local:removeHtmlTags($datasetIDtype)
                                else
                                    'other'"/>
                    </type>
                </dataset_id>
                <xsl:if test="$datasetIDtype/text() = 'other'">
                    <otherIdType>
                        <xsl:value-of
                ...
                <personal_data>
                    <xsl:value-of
                        select="//sections[number/text() = 5]/questions[number/text() = 1]/answer/options//text"
                    />
                </personal_data>
                <sensitive_data>
                    <xsl:value-of select="local:removeHtmlTags($ethicsNsensitive)"/>
                ...
                <xsl:variable name="Ttest" select="
                        for $i in $TableRowNo
                        return
                            substring-before(substring-after(//sections[number/text() = 10]/questions[number/text() = 1]/answer/text/text(), $i), '&lt;/em')"/>
                <xsl:choose>
                    <xsl:when
                        test="not(exists(//sections[number/text() = 10])) or string-length(local:removeHtmlTags(substring-before(substring-after(//sections[number/text() = 10]/questions[number/text() = 1]/answer/text/text(), 'T1:'), '&lt;/em'))) &lt; 1 or contains(substring-before(substring-after(//sections[number/text() = 10]/questions[number/text() = 1]/answer/text/text(), 'T1:'), '&lt;/em'), 'None')">
                        <!--   <dataset>  
                    ...
                    <xsl:when
                        test="string-length(substring-after(//sections[number/text() = 10]/questions[number/text() = 1]/answer/text/text(), 'T1:')) &gt; 1">
                        <!--    <dataset>  -->
                        <!--no. 1 -->
                        <title>
                ...
                <xsl:choose>
                    <xsl:when
                        test="not(exists(//sections[number/text() = 10])) or string-length(substring-after(//sections[number/text() = 10]/questions[number/text() = 1]/answer/text/text(), 'T2:')) &lt; 1 or contains(substring-before(substring-after(//sections[10]/questions[1]/answer/text/text(), 'T2:'), '&lt;/em'), 'None')"/>
                    <!--    <xsl:when
                test="local:removeHtmlTags(substring-before(substring-after(//sections[10]/questions[1]/answer/text/text(), 'T2:'), '&lt;/em')) != 'None'"> -->
                    <xsl:when
                        test="string-length(substring-after(//sections[number/text() = 10]/questions[number/text() = 1]/answer/text/text(), 'T2:')) &gt; 1">
                        <!--    <dataset>  -->
                        <!--no. 2 -->
                        <title>
                            <xsl:value-of
                ...
                <xsl:choose>
                    <xsl:when
                        test="not(exists(//sections[number/text() = 10])) or string-length(substring-after(//sections[number/text() = 10]/questions[number/text() = 1]/answer/text/text(), 'T3:')) &lt; 1 or contains(substring-before(substring-after(//sections[number/text() = 10]/questions[number/text() = 1]/answer/text/text(), 'T3:'), '&lt;/em'), 'None')"/>
                    <!--    <xsl:when
                test="local:removeHtmlTags(substring-before(substring-after(//sections[10]/questions[1]/answer/text/text(), 'T3:'), '&lt;/em')) != 'None'"> -->
                    <xsl:when
                        test="string-length(substring-after(//sections[number/text() = 10]/questions[number/text() = 1]/answer/text/text(), 'T3:')) &gt; 1">
                        <!--    <dataset>  -->
                        <!--no. 3 -->
                        <title>
                            <xsl:value-of
        ...
        <xsl:choose>
            <xsl:when
                test="//sections[number/text() = 2]/questions[number/text() = 6]/answer/options//text = 'yes'">
                <distribution>
                    <xsl:variable name="accessURLs">
                        <xsl:value-of
                            select="tokenize(//sections[number/text() = 2]/questions[number/text() = 7]/answer/text, ', ')"
                        />
                    </xsl:variable>
                    ...
                    <data_access>
                        <xsl:value-of
                            select="//sections[number/text() = 2]/questions[number/text() = 8]/answer/options//text"
                        />
                    </data_access>
                    <array>
                        <xsl:for-each select="
                                for $i in (tokenize(//sections[number/text() = 2]/questions[number/text() = 12]/answer/text, ', '))
                                return
                    ...
                    <array>
                        <license>
                            ...
                            <xsl:for-each
                                select="//sections[number/text() = 2]/questions[number/text() = 9]/answer/options//text">
                                <license_ref>
                                    <xsl:value-of select="."/>
                                </license_ref>
                                <start_date>
                            ...
                            <xsl:for-each
                                select="//sections[number/text() = 5]/questions[number/text() = 5]/answer/options//text">
                                <license_ref>
                                    <xsl:value-of select="."/>
                                </license_ref>
                                <start_date>
                    ...
                    <xsl:for-each select="
                        ...
                        <title>
                            <xsl:value-of
                                select="substring-before(., concat('.', tokenize(., '\.')[last()]))"
                            />
                        </title>
                    </xsl:for-each>
                </distribution>
    ...
    <xsl:template name="archivalVersions">
        ...
        <xsl:variable name="archiveFinal">
            <xsl:value-of
                select="//sections[number/text() = 10]/questions[number/text() = 3]/answer/options//text"
            />
        </xsl:variable>
        <finalVersion>
            <xsl:value-of select="
                    if ($archiveFinal = 'Yes') then
                        'Archive Final'
                    else
                        ()"/>
        </finalVersion>
        ...
        <FAIRscore>
            <sectionI>
                <DataDescription>
                    ...
                    <URL-idQ2>
                        <xsl:choose>
                            <xsl:when test="string-length($datasetIDstring) > 4">
                                <Text>
                        ...
                        <xsl:choose>
                            <xsl:when test="contains($datasetIDstring, 'http')">
                                <A-value>1</A-value>
                            </xsl:when>
                            <xsl:otherwise/>
                        </xsl:choose>
                    ...
                    <idTypeQ3>
                        ...
                        <xsl:choose>
                            ...
                            <xsl:when
                                test="contains($datasetIDstring, 'ark') and $datasetIDtype/text() = 'ark'">
                                <F-value>1</F-value>
                                <A-value>1</A-value>
                            </xsl:when>
                            <xsl:when
                                test="contains($datasetIDstring, 'doi') and $datasetIDtype/text() = 'doi'">
                                <F-value>1</F-value>
                                <A-value>1</A-value>
                                <FER>https://np.petapico.org/RAo3vAPCwEn47UOJMHT_QWhILBNmLpmM0mmlUrwHT7axU</FER>
                            </xsl:when>
                            <xsl:when
                                test="contains($datasetIDstring, 'handle') and $datasetIDtype/text() = 'handle'">
                                <F-value>1</F-value>
                                <A-value>1</A-value>
                                <FER>https://np.petapico.org/RAKJM2F3Q8pyY6MQNxFB980nkYhiiSZWDO_QfCbgY8eso</FER>
                            </xsl:when>
                            <xsl:when
                                test="contains($datasetIDstring, 'http') and $datasetIDtype/text() = 'url'">
                                <A-value>1</A-value>
                            </xsl:when>
                            <xsl:otherwise/>
                        </xsl:choose>
                    ...
                    <ReusedDatasetURL-licenseQ9>
                        <xsl:choose>
                            <xsl:when
                                ...
                                <Text>
                                    <xsl:value-of
                                        select="local:removeHtmlTags(//sections[number/text() = 2]/questions[number/text() = 9]/answer/text)"
                                    />
                                </Text>
                            </xsl:when>
                            <xsl:when
                ...
                <Metadata>
                    ...
                    <RepositoryQ1>
                        <xsl:variable name="optionText">
                            <xsl:value-of
                                select="//plan_content[1]//sections[number/text() = 3]/questions[number/text() = 1]/answer/options//text"
                            />
                        </xsl:variable>
            ...
            <sectionIII>
                <StorageQ1>
                    <xsl:for-each
                        select="//sections[number/text() = 4]/questions[number/text() = 1]/answer/options//text">
                        <xsl:variable name="checkNr">
                            <xsl:value-of select="
                                    for $i in (.)
                                    return
            ...
            <sectionIV>
                <PreventIllegitimateAccessQ2-Q3>
                    <xsl:choose>
                        <xsl:when
                            ...
                            <xsl:variable name="optionsQ">
                                <xsl:value-of select="
                                        for $i in (//sections[number/text() = 5]/questions[number/text() = 2]/answer/options//text)
                                        return
                                            $i"/>
                            </xsl:variable>
                            <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <xsl:for-each
                                select="//sections[number/text() = 5]/questions[number/text() = 3]/answer/options//text">
                                <Option>
                                    <xsl:value-of select="."/>
                                </Option>
                            </xsl:for-each>
                            <xsl:if
                                test="string-length((//sections[number/text() = 5]/questions[number/text() = 3]/answer/options//text)[1]) > 10">
                                <A-value>1</A-value>
                            </xsl:if>
                        </xsl:otherwise>
                    </xsl:choose>
                ...
                <EthicalReviewApplQ4>
                    <xsl:for-each
                        select="//sections[number/text() = 5]/questions[number/text() = 4]/answer/options//text">
                        <Option>
                    ...
                    <Text>
                        <xsl:value-of
                            select="local:replacEndash(local:removeHtmlTags(//sections[number/text() = 5]/questions[number/text() = 4]/answer/text))"
                        />
                    </Text>
                    <xsl:choose>
                        <xsl:when
                            test="contains(//sections[number/text() = 5]/questions[number/text() = 4]/answer/text, 'http')">
                            <F-value>1</F-value>
                            <A-value>1</A-value>
                        </xsl:when>
                        <xsl:when
                ...
                <LicenseQ5>
                    ...
                    <xsl:choose>
                        <xsl:when
                            test="contains(//sections[number/text() = 5]/questions[number/text() = 5]/answer/text, 'http')">
                            <A-value>1</A-value>
                            <R-value>1</R-value>
                        </xsl:when>
                        <xsl:when
            ...
            <sectionV>
                ...
                <AccessibleLocationQ1>
                    <xsl:for-each
                        select="//sections[number/text() = 6]/questions[number/text() = 1]/answer/options//text">
                        <xsl:variable name="checkNr">
                            <xsl:value-of select="
                                    for $i in (.)
                                    return
                ...
                <AccessibleContentQ2>
                    <xsl:for-each
                        select="//sections[number/text() = 6]/questions[number/text() = 2]/answer/options//text">
                        <Option>
                            <xsl:value-of select="."/>
                        </Option>
                        <xsl:choose>
                    ...
                    <xsl:choose>
                        <xsl:when
                            test="(//sections[number/text() = 6]/questions[number/text() = 2]/answer/options//text/text() = 'Other documentation') and (string-length(local:removeHtmlTags(//sections[number/text() = 6]/questions[number/text() = 2]/answer/text)) &gt; 4)">
                            <Text>
                                <xsl:value-of
                ...
                <SpecialSWtoAnalyseQ5>
                    <xsl:for-each
                        select="//sections[number/text() = 6]/questions[number/text() = 5]/answer/options//text">
                        <Option>
                            <xsl:value-of select="."/>
                        </Option>
                        <xsl:choose>
                ...
                <SpecialSWtoRerunQ6>
                    <xsl:for-each
                        select="//sections[number/text() = 6]/questions[number/text() = 6]/answer/options//text">
                        <Option>
                            <xsl:value-of select="."/>
                        </Option>
                        <xsl:choose>
                ...
                <SaaSinCloudQ8>
                    <xsl:for-each
                        select="//sections[number/text() = 6]/questions[number/text() = 8]/answer/options//text">
                        <Option>
                            <xsl:value-of select="normalize-space(.)"/>
                        </Option>
                    </xsl:for-each>
                ...
                <PersonalDataProcessAgreeSaaSQ9>
                    <xsl:for-each
                        select="//sections[number/text() = 6]/questions[number/text() = 9]/answer/options//text">
                        <Option>
                            <xsl:value-of select="normalize-space(.)"/>
                        </Option>
                        <xsl:choose>
            ...
            <sectionVIII>
                <AdministrativeInformation>
                    <contactIDtypeQ2>
                        <Option>
                            <xsl:value-of select="$contactIDtype"/>
                        </Option>
                        <xsl:choose>
                            <xsl:when
                                test="matches($contactIDstring, '\d{4}-\d{4}-\d{4}-\d{3}[\d|X]') and $contactIDtype = 'orcid'">
                                <F-value>1</F-value>
                                <A-value>1</A-value>
                                <FER>https://np.petapico.org/RAio8KwiLzN0pJ4Q2a_4VNjTe-liPr81B4jGIx_Cqqia8</FER>
                            </xsl:when>
                            <xsl:when
                                test="matches($contactIDstring, '\d{16}') and $contactIDtype/text() = 'isni'">
                                <A-value>1</A-value>
                            </xsl:when>
                            <xsl:when
                                test="contains($contactIDstring, 'viaf') and $contactIDtype/text() = 'other'">
                                <A-value>1</A-value>
                            </xsl:when>
                            <xsl:otherwise/>
                        </xsl:choose>
                        <xsl:choose>
                            <xsl:when
                                test="$contactIDtype/text() = 'other' and string-length(//sections[number/text() = 9]/questions[number/text() = 2]/answer/text) > 4">
                                <Text>
                                    <xsl:value-of
                                        select="local:removeHtmlTags(//sections[number/text() = 9]/questions[number/text() = 2]/answer/text)"
                                    />
                                </Text>
                                <F-value>1</F-value>
                            </xsl:when>
